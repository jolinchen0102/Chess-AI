    ...
    def moves(cords, color, chessboard = None):
        ...
        if chessboard:
            ...
            if color == 'w':
                ...
                if rules.RulesEnforcer.collision_detection(move1, color, chessboard) not in ["friend","enemy"]:
                ...
                if int(cords[1]) == 2 and obstructed == False:
                    ...
                    if rules.RulesEnforcer.collision_detection(move2, color, chessboard) not in ["friend","enemy"]:
        ...
        if chessboard:
            board_cords = rules.RulesEnforcer.coordinate_mapper(cords)
            if color == 'w':
                #check diagonal left
                if board_cords[0] > 0 and board_cords[1] > 0:
                    ...
                    if square.split('-')[0] == 'b':
                        final_cord = rules.RulesEnforcer.coordinate_mapper_reverse([row,column])
        ...
        pos_moves = rules.RulesEnforcer.remove_outofbound_moves(pos_moves)
        return pos_moves
...
class Rook(object):
    ...
    def moves(cords, color, chessboard = None):
        ...
        while x_new >= 97 and x_new <= 104 and y_new >= 1 and y_new <= 8:
            ...
            if chessboard:
                if rules.RulesEnforcer.collision_detection(move, color, chessboard) == "friend":
                    break
                elif rules.RulesEnforcer.collision_detection(move, color, chessboard) == "enemy":
